...
import { useFetchProvince } from "@/config/fetch";
import { useClickOutside } from "@/hooks";
import { ChevronDown, Loader, MapPin, Search, X } from "lucide-react";
...
export function HeaderSearch() {
  ...
  const { data, isFetching } = useFetchProvince();
  const ref = useRef<HTMLDivElement>(null);

  const filteredProvinces = data.filter(province =>
    province.name.toLowerCase().includes(provinceSearchText.toLocaleLowerCase())
  );
    ...
    <div className="flex-1 py-2 px-2 sm:py-2.5 sm:pl-6 sm:pr-8 w-full">
      ...
      {isProvinceChooserShown && (
        <div className="fixed z-10 inset-0 flex justify-center items-center bg-black/30 p-2">
          <div ref={ref} className="max-w-[480px] max-h-[700px] h-full w-full bg-white p-4 shadow-lg rounded-sm">
            {isFetching ? (
              <div className="flex justify-center items-center h-full">
                <Loader size={30} className="animate-spin text-app-primary" />
              </div>
            ) : (
              <div className="h-full flex flex-col">
                <div className="flex items-center">
                  <X className="cursor-pointer" onClick={e => handleToggleProvinceChooser(e, false)} />
                ...
                <div className="flex flex-1 flex-col overflow-y-auto pb-2">
                  {filteredProvinces.map(province => (
                    ...
                    >
                      ...
                      <input type="radio" className="accent-app-secondary" id={province.name} name="province" />
                    </label>
                  ))}
                </div>
